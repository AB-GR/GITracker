...
namespace SQLiteNetExtensions
{
	using Attributes;
	using Extensions.TextBlob;
	using System;
	...
	public static class ReadOperations
	{
		...
		public static List<T> GetAllWithChildrenOptimized<T>(this SQLiteConnection conn, Expression<Func<T, bool>> filter = null)
			where T : new()
		{
			var elements = conn.Table<T>();
			if (filter != null)
			...
			conn.GetChildrenOptimized(list);
		...
		private static void GetChildrenOptimized<T>(this SQLiteConnection conn, IList<T> elements)
		{
			var element = elements.FirstOrDefault();
			if (element == null)
				return;

			var relationshipProperties = element.GetType().GetRelationshipProperties();
			foreach (var relationshipProperty in relationshipProperties)
			{
				var relationshipAttribute = relationshipProperty.GetAttribute<RelationshipAttribute>();
				if (relationshipAttribute.IsCascadeRead)
				{
					GetChildrenOptimized(conn, elements, relationshipProperty);
				}
			}
		}

		private static void GetChildrenOptimized<T>(this SQLiteConnection conn, IList<T> elements, PropertyInfo relationshipProperty)
		{
			var relationshipAttribute = relationshipProperty.GetAttribute<RelationshipAttribute>();

			if (relationshipAttribute is OneToManyAttribute)
			{
				conn.GetOneToManyChildren(elements, relationshipProperty);
			}
			else if (relationshipAttribute is ManyToOneAttribute)
			{
				conn.GetManyToOneChildren(elements, relationshipProperty);
			}
		}

		private static void GetManyToOneChildren<T>(this SQLiteConnection conn, IList<T> elements,
			PropertyInfo relationshipProperty)
		{
			var primaryKeys = new Dictionary<object, IList<T>>();
			var type = elements[0].GetType();
			...
			{
				var placeHolders = string.Join(",", Enumerable.Repeat("?", primaryKeys.Count));
				var query = $"select * from [{tableMapping.TableName}] where [{tableMapping.PK.Name}] in ({placeHolders})";
				IList<object> values = conn.Query(tableMapping, query, primaryKeys.Keys.ToArray());
				...
				if (values.Count > 0)
				{
					...
					{
						var keyValue = keyProperty.GetValue(value);
						if (primaryKeys.TryGetValue(keyValue, out var keyElements))
						{
							foreach (var keyElement in keyElements)
							{
								relationshipProperty.SetValue(keyElement, value, null);
		...
		private static void GetOneToManyChildren<T>(this SQLiteConnection conn, IList<T> elements,
			PropertyInfo relationshipProperty)
		{
			var primaryKeys = new Dictionary<object, IList<T>>();
			var type = elements[0].GetType();
			...
			foreach (T element in elements)
			{
				var foreignKeyValue = currentEntityPrimaryKeyProperty.GetValue(element, null);
				if (foreignKeyValue != null)
				{
					AddPrimaryKeyToDictionary<T>(foreignKeyValue, element, primaryKeys);
				}
			...
			if (primaryKeys.Count > 0)
			{
				var placeHolders = string.Join(",", Enumerable.Repeat("?", primaryKeys.Count));
				var query =
					$"select * from [{entityType.GetTableName()}] where [{otherEntityForeignKeyProperty.GetColumnName()}] in ({placeHolders})";
				IList<object> queryResults = conn.Query(tableMapping, query, primaryKeys.Keys.ToArray());

				var otherEntityForeignKey = entityType.GetRuntimeProperty(otherEntityForeignKeyProperty.GetColumnName());
				foreach (T element in elements)
				{
					var foreignKeyValue = currentEntityPrimaryKeyProperty.GetValue(element, null);

					// Get all values from query results which corresponds to the element one by one.
					var otherEntityResults = queryResults.Where(x => otherEntityForeignKey.GetValue(x).ToString().Equals(foreignKeyValue.ToString())).ToList();
					...
					if (enclosedType == EnclosedType.List)
						...
						values = array = Array.CreateInstance(entityType, otherEntityResults.Count);

					int i = 0;
					foreach (var result in otherEntityResults)
					{
						if (array != null)
